...
// Command lấy danh sách nhân viên từ API
Cypress.Commands.add('getEmployeesFromAPI', () => {
  return cy.request({
    method: 'GET',
    url: `${Cypress.env('apiUrl')}/api/staffs/search`, // Đường dẫn API lấy danh sách nhân viên
    failOnStatusCode: false // Tùy chọn: không fail test nếu API trả về lỗi
  }).then((response) => {
    if (response.status === 200) {
      return response.body;
    } else {
      cy.log(`API trả về status: ${response.status}`);
